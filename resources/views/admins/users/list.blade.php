                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header d-flex align-items-center">
                            ...
                            <a href="{{ route('create.user') }}" class="btn btn-primary btn-round ms-auto">
                                        ...
                                        @foreach ($users as $item)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center"><span
                                                        class="mx-auto badge {{ $bgRole }}">{{ $item->role->name }}</span>
                                                </td>
                                                <td class="text-center" id="status-{{ $item->id }}"><span
                                                        class="mx-auto badge {{ $bgStatus }}">{{ $textStatus }}</span>
                                                </td>
                                                <td>
                                                    @if ($item->role->name !== 'Super Admin')
                                                        <div class="d-flex justify-content-center">
                                                            <button data-bs-toggle="tooltip" title="Kích hoạt"
                                                            ...
                                                            <button data-bs-toggle="tooltip" title="Vô hiệu hóa"
                                                                data-id="{{ $item->id }}" data-status="disabled"
                                                                class="btn btn-link btn-danger btn-lg btn-status"
                                                                data-original-title="Edit Task"><i
                                                                    class="fas fa-user-lock"></i></button>
                                                            <a href="{{ route('show.user', ['id' => $item->id]) }}"
                                                                data-bs-toggle="tooltip" title="Xem chi tiết"
                                                                data-id="{{ $item->id }}" data-status="detail"
                                                                class="btn btn-link btn-warning btn-lg"
                                                                data-original-title="Edit Task"><i
                                                                    class="fas fa-eye"></i></a>
                                                        </div>
                                                    @else
                                                        <p class="badge badge-info">Bạn không thể thao tác được người này!
                                                        </p>
                                                    @endif
                                                </td>
                                            </tr>
