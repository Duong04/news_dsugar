    ...
    <div class="page-inner">
        ...
        <div class="row">
          <form class="row col-12" action="{{ route('update.subcategory', ['id' => $subcategory->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <x-form.input2 :value="$subcategory->name" :error="$errors->first('name')" name="name" label="Tên danh mục" type="text" />
            <x-form.input2 :value="$subcategory->description" :error="$errors->first('description')" name="description" label="Mô tả ngắn" type="text" />
            <x-form.select :id="$subcategory->category_id" type="categories" :error="$errors->first('category_id')" name="category_id" label="Danh mục con" />
